...
```sql
SELECT r.id, r.name, r.address, r.active
FROM restaurants r
...
### Počet restaurací s menu podle měst a dne
```sql
SELECT c.name AS city, dm.date, COUNT(DISTINCT r.id) AS restaurants
FROM daily_menus dm
JOIN restaurants r ON r.id = dm.restaurant_id
JOIN cities c ON c.id = r.city_id
GROUP BY c.name, dm.date
